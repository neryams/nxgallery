...
export class InputFileComponent implements ControlValueAccessor, OnInit {
  ...
  @Input() set fileLimit(fileLimit: number) {
    this._fileLimit = typeof fileLimit === 'string' ? parseInt(fileLimit, 10) : fileLimit;
  }
  ...
  constructor(private readonly formBuilder: FormBuilder, private readonly inputFileService: InputFileService) {}
  ...
  onSelectFile(fileList: FileList, button: MatButton): void {
    if (!this.disabled) {
      ...
      // Copies the array without reference.
      const files = this.files.slice();
      if (this.fileLimit === 1 && files.length === 1) {
        this.onReplaceFile(fileList, 0, button);
      } else {
        Array.from(fileList).forEach(file => {
          const inputFile: InputFile = { file };
          // If the file limit is exactly 1 and there is a file currently selected, replace it.
